...
class wfCart {
	...
	function plus_item($itemid,$qty=1,$price = FALSE, $name = FALSE, $desc = FALSE, $image = FALSE)
	{ 
		...
		if(@$this->itemqtys[$itemid] > 0) { 
			...
			$this->itemimage[$itemid] = $image;
			$this->itemname[$itemid] = $name;
			$this->itemdesc[$itemid] = $desc;
		}
		$this->_update_total();
	} // end of plus_item
	
	
	function minus_item($itemid)
	{ 
	// changes an items quantity

		 
		$this->itemqtys[$itemid] = $this->itemqtys[$itemid]-1;
			
		if($this->itemqtys[$itemid] < 1) {
