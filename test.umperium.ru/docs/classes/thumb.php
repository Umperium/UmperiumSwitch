...
if(! defined('FILE_CACHE_DIRECTORY') ) 		define ('FILE_CACHE_DIRECTORY', '../uploads/_thumbs/Media');				// Directory where images are cached. Left blank it will use the system temporary directory (which is better for security)
...
if(! defined('WATERMARK_OVERLAY_IMAGE') )	define( 'WATERMARK_OVERLAY_IMAGE', 'watermark.png'); // the watermark image
...
if ($watermark == 1) {
		...
		imagecopy( $canvas, $overlay_gd_image, ($new_width -$overlay_width-5), ($new_height -$overlay_height-5), 0, 0, $overlay_width, $overlay_height);
		imagealphablending($canvas, false );
		imagesavealpha($canvas , true);
}
